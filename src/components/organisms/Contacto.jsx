...
export default function Contacto() {
  ...
  const EMAIL_RE =
    /^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/i; // simple y suficiente para frontend
  ...
  const errors = useMemo(() => {
    ...
    if (!values.email.trim()) {
      ...
      e.email = 'Formato de email no válido (ej.: nombre@dominio.cl).';
    }

    // Descripción
    const issueLen = values.issue.trim().length;
    if (issueLen === 0) {
      e.issue = 'Describe el problema para poder ayudarte.';
    } else if (issueLen < 10) {
      e.issue = 'Cuenta un poco más de detalle (mínimo 10 caracteres).';
    } else if (issueLen > 500) {
      e.issue = 'Demasiado largo (máximo 500 caracteres).';
    }

    // Pedido opcional
    if (values.order.trim()) {
      if (!ORDER_RE.test(values.order.trim())) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    // marca todo como tocado para mostrar justificaciones
    ...
    try {
      ...
      setStatus({
        type: 'error',
        msg: 'No se pudo enviar. Intenta de nuevo en un momento.',
      });
    }
  };
      ...
      >
        ...
        <div className={`field ${touched.name && errors.name ? 'has-error' : ''}`}>
          ...
          <small id="name-help" className="hint">
            Tu nombre y apellido (3–60 caracteres).
          </small>
        ...
        <div className={`field ${touched.issue && errors.issue ? 'has-error' : ''}`}>
          ...
          <textarea
            ...
            rows={5}
          />
          <small id="issue-help" className="hint">
            Sé concreto: ¿qué intentabas hacer?, ¿qué pasó?, ¿sale algún error?
          </small>
          <small id="issue-error" className="error">
            {touched.issue && errors.issue ? errors.issue : ''}
          </small>
        </div>
        ...
        <div className={`field ${touched.order && errors.order ? 'has-error' : ''}`}>
          ...
          />
          <small id="order-hint" className="hint">
            Formato: 2–3 letras + 3 números (p. ej., JM001 o ABC123).
          </small>
          <small id="order-error" className="error">
